...
In R, strings are of the basic type character and they can be created by using quotation marks or specific functions. There are quite a few functions in Base R that can be used to manipulate characters, but there is also a bit more consistent and simple tidyverse package **stringr.**
...
# False means they are not complete ie. they have NA and True means they are complete?
...
AFTER We have a high-dimensional space and we want to reduce that dimenson

also lose a little bit of data when going from high to low dimensional space
...
biplot(pca_human, choices = 1:2, cex = 0.5, col = c("grey40", "deeppink2"))
...
pce_per <- round(pca_pr*100, digits = 1)
pce_per
...
# draw a biplot
biplot(pca_human, cex = c(0.8, 1), col = c("grey40", "deeppink2"), labs = pc_lab)
```
