...
data will usually be in R `data.frame` format. The dimensions of a data
...
lrn14$attitude <- lrn14$Attitude / 10 # divided by 10 bc 10 qs 
...
lrn14$strat <- rowMeans(strategic_columns)
...
# add a main title
p4 <- p3 + ggtitle("Student's attitudes versus exam points")
...
#Add the argument col to the pairs() function, defining the colour with the 'gender' variable in learning2014.
pairs(learning2014, col = gender)

# ^ cannot figure out why gender is not being found, from pairs documentation col is not a parameter, it is in graphical parameter which I cannot figure out 
...
# add gender as col
p_gen <- ggpairs(learning2014, mapping = aes(col = gender), lower = list(combo = wrap("facethist", bins = 20)))

# draw the plot
p_gen # can see that the plot is now coloured by gender 

p_alph <- ggpairs(learning2014, mapping = aes(col = gender, alpha = 0.3), lower = list(combo = wrap("facethist", bins = 20)))

p_alph # alpha adjusts transparency 
...
# create a regression model with multiple explanatory variables
my_model3 <- lm(points ~ attitude + surf, data = learning2014)
...
par(mfrow = c(2,2)) # FOR PAR TO WORK YOU NEED TO RUN THE WHOLE CODE CHUNK 
#plot(1:10) # testing plots for par
#plot(2:10) # testing plots for par
...
# okay wait so this model is really bad at prediciting? they are off by over 20, am I missing something here? 
