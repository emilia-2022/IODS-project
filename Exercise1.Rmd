...
gbd_short %>% 
 filter(year == 1995) # needs to be double equals 
...
p3 <- europe %>%
  ...
  theme(legend.position = "none")
library(patchwork)
p1 / p2 | p3 # puts two plots above each other and one on the side 
p1 / p2 / p3 # puts all plots on top of each other 
...
```{r}
# filter data 
europe_2years <- europe %>%
  filter(year %in% c(1962, 1977))
view(europe)
view(europe_2years)
...
meldata %>% 
  summary_factorlist(dependent = "status.factor", 
                     explanatory = 
                       c("ulcer.factor", "age.factor", 
                         "sex.factor", "thickness"), 
                     cont = "median" # gives median and IQR range
  )
...
```{r}
# I cannot do this one, I do not understand the code 
